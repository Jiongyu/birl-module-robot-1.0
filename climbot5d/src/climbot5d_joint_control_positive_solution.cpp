...
int main(int argc, char **argv)
{
    ...
    Kine_CR_FiveDoF_G1 Climbot_G0; 
    Kine_CR_FiveDoF_G2 Climbot_G6; 
    ...
    ros::init(argc, argv, "climbot5d_joint_control_positive_solution");
    ros::NodeHandle nh;
    ros::Publisher pub = nh.advertise<std_msgs::Float64MultiArray>("climbot5d_Positive_solution", 1);
    ros::Subscriber sub = nh.subscribe("climbot5d_Joint_point", 1, subCallback);
    ...
    while(ros::ok()){
        ...
        if ((fabs(actual_joint_value[0] - request_joint_point.data[1]*PI_DEG))>1e-3 ||
            (fabs(actual_joint_value[1] - request_joint_point.data[2]*PI_DEG))>1e-3 ||
            (fabs(actual_joint_value[2] - request_joint_point.data[3]*PI_DEG))>1e-3 ||
            (fabs(actual_joint_value[3] - request_joint_point.data[4]*PI_DEG))>1e-3 ||
            (fabs(actual_joint_value[4] - request_joint_point.data[5]*PI_DEG))>1e-3){
                
                actual_joint_value[0] = request_joint_point.data[1]*PI_DEG;
                actual_joint_value[1] = request_joint_point.data[2]*PI_DEG;
                actual_joint_value[2] = request_joint_point.data[3]*PI_DEG;
                actual_joint_value[3] = request_joint_point.data[4]*PI_DEG;
                actual_joint_value[4] = request_joint_point.data[5]*PI_DEG;
                
                if (request_joint_point.data[0])
                    Climbot_G0.FKine(actual_joint_value,descartes_point);
                else
                    Climbot_G6.FKine(actual_joint_value,descartes_point);
